...
import { changeItemQuantity, removeItem, clearCart } from './cartSlice';
  ...
  return (
    <div id="cart-container">
      ...
      {Object.keys(cart).length > 0 && (
        <button id="cart-clear-button" onClick={() => dispatch(clearCart())}>Clear Cart</button>
      )}
      <h3 className="total">
        Total{' '}
  ...
  function createCartItem(name) {
    ...
    return (
      <li key={name}>
        ...
        <button id="remove-item-button" onClick={() => dispatch(removeItem(name))}>x</button>
      </li>
    );
  }
};
